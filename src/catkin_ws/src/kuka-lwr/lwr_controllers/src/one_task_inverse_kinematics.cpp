...
namespace lwr_controllers 
{
    ...
    bool OneTaskInverseKinematics::init(hardware_interface::PositionJointInterface *robot, ros::NodeHandle &n)
    {
        ...
        x_des_ = x_;
        float tool_length;
        // tool_length=0.22; //0.22 is tool tip length
        //tool_length=0.0;
        tool_length=0.275;//0.330
        float init_tip[]={};
    ...
    void OneTaskInverseKinematics::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        if (cmd_flag_)
        {
            ...
            // tool_length=0.22;
            // tool_length=0.0;
            tool_length=0.275; // Real length for Tool3
            //tool_length=0.330; 
            ...
            for (int i = 0; i < joint_handles_.size(); i++)
                ...
                joint_des_states_.q(i) += 0.05*joint_des_states_.qdot(i); // 0.01,JLF 2022/06/08 
            ...
            if (Equal(x_, x_des_new, 0.0005)) // orignal 0.05, should be wrong
            {
                x_.M.GetRPY(end_orientation(0),end_orientation(1),end_orientation(2));
                ROS_INFO("On target");
